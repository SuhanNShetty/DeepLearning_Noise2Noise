...
class SGD(object):
	...
	def step(self):
		...
		for i,tfm in enumerate(self.transforms):
			# set momentum
			self.velocity[i][0] = self.damping*self.velocity[i][0] + self.params[i].grad_weight # weight
			self.velocity[i][1] = self.damping*self.velocity[i][1] + self.params[i].grad_bias  # bias
